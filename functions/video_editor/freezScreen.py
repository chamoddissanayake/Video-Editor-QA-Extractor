...
def generate_timestamps(noise, duration):
    cmd = "ffmpeg -hide_banner -nostats -an  -i " + input_path + " -vf freezedetect=n=-" + noise + "dB:d=" + duration + " -map 0:v:0  -f null - 2>&1 "
...
def mainVideo(noise, duration):
    count = 0
    last = 0
    timestamps = generate_timestamps(noise, duration)
